...
import os
obesity_df = pd.read_csv(r"C:\Users\prevo\OneDrive\Documents\GitHub\CSC-212-Data-Structs-Alg-Labs\Final-Project\ObesityDataSet_raw_and_data_sinthetic.csv")
obesity_df = obesity_df.sample(frac=1).reset_index(drop=True) # Shuffle

categorical_features = obesity_df[['Gender', 'family_history_with_overweight', 'FAVC', 'CAEC', 'SMOKE', 'SCC', 'CALC', 'MTRANS']]
categorical_encoder = OneHotEncoder(sparse_output=False)
categorical_encoded = categorical_encoder.fit_transform(categorical_features)
X_numeric = obesity_df[['Age', 'Height', 'Weight', 'FCVC', 'NCP', 'CH2O', 'FAF', 'TUE']]
X = np.hstack((np.array(X_numeric), categorical_encoded))
X[:5]
